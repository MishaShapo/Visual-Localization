...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, std::vector<BoundingBox> &bBoxes)
{
    if(mSensor!=RGBD)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageRGBD(im,depthmap,timestamp, bBoxes);
